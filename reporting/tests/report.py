...
from tests.utils import post_to_km_endpoint
...
KM_URL = os.environ.get("KNOWLEDGE_MIDDLEWARE_URL", "http://knowledge-middleware-api:8000")
...
def pdf_to_cosmos(scenario):
    km_response = post_to_km_endpoint("/pdf_to_cosmos", json={"document_id": scenario})

    if km_response.status_code != 200:
        raise Exception(f"Knowledge Middleware returned {km_response.status_code} for 'pdf to cosmos' on scenario: {scenario}")
...
def pdf_to_text(scenario):
    km_response = post_to_km_endpoint("/pdf_extractions", json={"document_id": scenario})

    if km_response.status_code != 200:
        raise Exception(f"Knowledge Middleware returned {km_response.status_code} for 'pdf to text' on scenario: {scenario}")
...
def code_to_amr(scenario):
    ...
    km_response = post_to_km_endpoint("/code_to_amr", json={"code_id": scenario, "dynamics_only": True})

    if km_response.status_code != 200:
        raise Exception(f"Knowledge Middleware returned {km_response.status_code} for 'code to amr' on scenario: {scenario}")
...
def profile_model(scenario, model_id):
    km_response = post_to_km_endpoint(f"/profile_model/{model_id}", json={"document_id": scenario})

    if km_response.status_code != 200:
        raise Exception(f"Knowledge Middleware returned {km_response.status_code} for 'profile model' on scenario: {scenario}")
...
def link_amr(scenario, model_id):
    km_response = post_to_km_endpoint("/link_amr", json={"document_id": scenario, "model_id": model_id})

    if km_response.status_code != 200:
        raise Exception(f"Knowledge Middleware returned {km_response.status_code} for 'link amr' on scenario: {scenario}")
